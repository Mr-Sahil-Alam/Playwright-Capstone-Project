...
- Name: checkout\checkoutProductsLink.spec.js >> Verify Products Link Visible On Checkout Page
- Location: tests\checkout\checkoutProductsLink.spec.js:5:1
...
- banner:
  ...
  - text: for more information. 2026-05-29 14:51:10 UTC
- text: You
...
```ts
  ...
  4  | 
  5  | test('Verify Products Link Visible On Checkout Page', async ({ page }) => {
  6  | 
  ...
  12 | 
  13 |     await expect(
  14 |         page.getByRole('link', { name: 'Products' })
> 15 |     ).toBeVisible();
     |       ^ Error: expect(locator).toBeVisible() failed
  16 | 
  17 | });
