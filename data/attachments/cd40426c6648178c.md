...
- Name: checkout\checkoutHomeLink.spec.js >> Verify Home Link Visible On Checkout Page
- Location: tests\checkout\checkoutHomeLink.spec.js:5:1
...
- banner:
  ...
  - text: for more information. 2026-05-29 14:50:34 UTC
- text: You
...
```ts
  ...
  4  | 
  5  | test('Verify Home Link Visible On Checkout Page', async ({ page }) => {
  6  | 
  ...
  12 | 
  13 |     await expect(
  14 |         page.getByRole('link', { name: 'Home' }).first()
> 15 |     ).toBeVisible();
     |       ^ Error: expect(locator).toBeVisible() failed
  16 | 
  17 | });
