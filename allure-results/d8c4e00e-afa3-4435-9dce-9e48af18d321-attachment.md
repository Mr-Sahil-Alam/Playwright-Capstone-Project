...
Locator: locator('.cart_description h4 a').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.cart_description h4 a').first()
...
```ts
  ...
  4  | 
  5  | test('Verify Product Visibility In Cart', async ({ page }) => {
  6  | 
  ...
  18 | 
  19 |     await expect(
  20 |         productPage.cartProduct.first()
> 21 |     ).toBeVisible();
     |       ^ Error: expect(locator).toBeVisible() failed
  22 | 
  23 | });
