...
Locator: getByRole('link', { name: 'API Testing' })
Expected: visible
Timeout: 5000ms
...
- banner:
  ...
  - text: for more information. 2026-05-23 05:24:18 UTC
- text: You
...
```ts
  ...
  4  | 
  5  | test('Verify API Testing Link Visible On Checkout Page', async ({ page }) => {
  6  | 
  ...
  12 | 
  13 |     await expect(
  14 |         page.getByRole('link', { name: 'API Testing' })
> 15 |     ).toBeVisible();
     |       ^ Error: expect(locator).toBeVisible() failed
  16 | 
  17 | });
