...
- Name: checkout\checkoutSubscription.spec.js >> Verify Subscription Section On Checkout Page
- Location: tests\checkout\checkoutSubscription.spec.js:5:1
...
- banner:
  ...
  - text: for more information. 2026-05-29 13:30:59 UTC
- text: You
...
```ts
  ...
  4  | 
  5  | test('Verify Subscription Section On Checkout Page', async ({ page }) => {
  6  | 
  ...
  12 | 
  13 |     await expect(
  14 |         page.getByText('Subscription')
> 15 |     ).toBeVisible();
     |       ^ Error: expect(locator).toBeVisible() failed
  16 | 
  17 | });
